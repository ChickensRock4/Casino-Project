    ...
    while (true) {
        ...
        if (betPlaced) {
            ...
            if (playerWins) {
                ...
                string output = "You lost $" + to_string(betAmount) + "\n";
...
void Blackjack::placeBet(double bet) {
    player->purchase(bet);
    string output = "You placed a Bet of " + to_string(bet) + "\n";
...
void Blackjack::payoutWin(double amnt) {
    ...
    string output = "Congratulations! You won $" + to_string(amnt) + "\n";
    ColorOutput::colorizeConsoleOutput(BRIGHT_GREEN, output);

    string newBalOutput = "You have a new balance of $" + to_string(player->getWallet()) + "\n";
